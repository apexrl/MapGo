	...
	def __init__(self, shape, sess, eps_std=1e-2, norm_clip=5):
		...
		with tf.variable_scope('normalizer_variables', initializer=tf.zeros_initializer()):
			self.sum = tf.get_variable(
				name='sum', shape=self.shape, dtype=np.float32, trainable=False)
			self.sum_sqr = tf.get_variable(
				name='sum_sqr', shape=self.shape, dtype=np.float32, trainable=False)
			self.cnt = tf.get_variable(
				name='cnt', shape=[1], dtype=np.float32, trainable=False)
			self.mean = tf.get_variable(
				name='mean', shape=self.shape, dtype=np.float32, trainable=False)
			self.std = tf.get_variable(
				name='std', shape=self.shape, dtype=np.float32, trainable=False)
		...
		self.update_scalar_op = tf.group(
			...
			self.std.assign(tf.maximum(self.eps_std, tf.sqrt(
				self.sum_sqr/self.cnt-tf.square(self.sum/self.cnt))))
		)
